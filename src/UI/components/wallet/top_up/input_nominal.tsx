...
const TopUpInputNominal: React.FC<TopUpInputNominalProps> = ({
  amount,
  setAmount,
  setIsOpenBankWallet,
  calculateReceived,
          ...
          <div className="grid grid-cols-2 gap-3">
            {walletTypes.map((wallet) => (
              <button
                ...
                className={`topup-card rounded-lg p-3 text-left ${
                  selectedWallet === wallet.id ? "selected" : ""
                }`}
              >
                <div
                  className={`w-8 h-8 bg-gradient-to-r ${wallet.color} rounded-lg mb-2 flex items-center justify-center`}
                >
